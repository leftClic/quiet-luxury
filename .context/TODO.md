# Quiet Luxury - To-Do List (TODO)

## Active / In Progress Tasks


## Pending Tasks (Backlog)

- [ ] Implement basic user authentication.
- [ ] Develop a luxury product catalog.

## Completed Tasks

- [x] **Establish the connection with Supabase and persist inquiries**
- [x] **Set up the basic Node.js server in `/backend`**
- [x] Create the `AGENTS.md` file with the main rules.
- [x] Create the `.context/` folder and its initial files (`CONTEXT.md`, `ARCHITECTURE.md`, `TODO.md`).
- [x] Install and configure the context7 MCP tool at the project level (opencode.json, AGENTS.md, skill).
- [x] Install and configure the necessary tools/skills (/frontend-design).
- [x] Design the frontend layout using Vanilla HTML/CSS (index.html, style.css, app.js in /frontend).
